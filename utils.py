...
from modules.renderers.gaussians_renderer import Simple_Camera,MiniCam
...
def sample_from_two_colmap_camera(colmap_camera_a, colmap_camera_b, fraction, noise_strengths=[0, 0]):
    """
    Args:
        pose_a: first pose
        pose_b: second pose
    ...
    quat_a = matrix_to_quaternion(torch.from_numpy(colmap_camera_a.R).cuda())
    quat_b = matrix_to_quaternion(torch.from_numpy(colmap_camera_b.R).cuda())

    quaternion = quaternion_slerp(quat_a, quat_b, fraction)
    quaternion = torch.nn.functional.normalize(quaternion + torch.randn_like(quaternion) * noise_strengths[0], dim=-1)

    R = quaternion_to_matrix(quaternion)
    T = (1 - fraction) * torch.from_numpy(colmap_camera_a.T).cuda() + fraction * torch.from_numpy(colmap_camera_b.T).cuda()
    T = T + torch.randn_like(T) * noise_strengths[1]

    new_camera = Simple_Camera(colmap_id=colmap_camera_a.uid,R=R.squeeze(0).cpu().numpy(),T=T.squeeze(0).cpu().numpy(),
                               FoVx=colmap_camera_a.FoVx,FoVy=colmap_camera_a.FoVy,
                               h=colmap_camera_a.image_height,w=colmap_camera_a.image_width,
                               qvec=colmap_camera_a.qvec,image_name=colmap_camera_a.image_name,uid=colmap_camera_a.uid,
                               render_res_factor=colmap_camera_a.render_res_factor
                               )
    return new_camera

def sample_from_colmap_cameras(colmap_cameras, t, noise_strengths=[0, 0, 0, 0]):
    N = len(colmap_cameras)

    left = torch.floor(t * (N-1)).long().clamp(0, N-2)
    right = left + 1
    fraction = t * (N-1) - left
    #camera_list = random.sample(colmap_cameras, 2)
    new_camera = sample_from_two_colmap_camera(colmap_cameras[left],colmap_cameras[right],fraction,noise_strengths=noise_strengths[:2])
...
# 导出点云数据
def export_points_for_gaussians(file_path, xyz, masks_3d, default_color=(255, 255, 0), delta_xyz=None,  use_triplane=False):
    ...
    rgb_values = np.tile(default_color, (xyzs.shape[0], 1))
    rgb_values[masks_3d] = original_color
    
    if delta_xyz is not None:
        delta_xyz = delta_xyz.detach().cpu().numpy()
        xyzs_transformed = xyzs[masks_3d] + delta_xyz[masks_3d]
        rgb_values_transformed = np.tile(target_color, (xyzs_transformed.shape[0], 1))
        
        xyzs = np.concatenate((xyzs, xyzs_transformed))
        rgb_values = np.concatenate((rgb_values, rgb_values_transformed))
    
    # 创建 Trimesh 点云对象
    cloud = trimesh.points.PointCloud(vertices=xyzs, colors=rgb_values/255.0)
    cloud.export(file_path)
    print(f"Point cloud saved to {file_path}")

# 导出两个mask的点云数据
def export_points_for_gaussians_with_two_masks(file_path, xyz, masks_3d, masks_3d_2, default_color=(255, 255, 255)):
    
    target_color = [255, 0, 0]
    original_color = [0, 255, 0]
    
    masks_3d = masks_3d.detach().cpu().numpy()
    masks_3d_2 = masks_3d_2.detach().cpu().numpy()
    means3D = xyz.contiguous().float()
    ...
    rgb_values[masks_3d_2] = target_color
...
#转换director3d相机pose
def convert_camera_parameters_into_viewpoint_cameras(cameras, h=None, w=None,image_name="", render_res_factor=1):
        device = cameras.device
        cameras = cameras.cpu()
        c2w = torch.eye(4)
        ...
        cam = MiniCam(c2w.numpy(), w, h, fovy.numpy(), fovx.numpy(), 0.1, 100,image_name=image_name, device=device,render_res_factor=render_res_factor)
        return cam
